...
void maxProfitScheduleHelper(const Set<Shift>& totalShifts, int maxHours, Set<Shift>& chosenShifts,
    ...
    // inclusion
    if((maxHours - lengthOf(chosenShift)) >= 0 && (chosenShifts.isEmpty() || !overlapsWith(chosenShift, chosenShifts.last()))) {
        chosenShifts += chosenShift;
        ...
        maxProfitScheduleHelper(totalShifts - chosenShift, maxHours - lengthOf(chosenShift), chosenShifts, chosenProfit,
                                maxProfitShifts, maxProfit);
        chosenShifts -= chosenShift;
    }
}
 ...
 */
STUDENT_TEST("numSchedulesFor works with no shift.") {
    Set<Shift> shifts = {};

    EXPECT_EQUAL(numSchedulesFor(shifts, 24), 0);
}
