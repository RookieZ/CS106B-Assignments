...
void depthsOfHelper(Node* root, Map<Node*, int>& map, int depth) {
    map.put(root, depth);
    if(root->children.size() != 0) {
        depth++;
        for(Node* child : root->children) {
            depthsOfHelper(child, map, depth);
        }
        depth--;
    }
}
...
Map<Node*, int> depthsOf(Node* root) {
    ...
    depthsOfHelper(root, result, depth);

    return result;
}

void partitionChildrenOfHelper(Vector<Node*> nodes, const Rectangle& bounds, Map<Node*, Rectangle>& map) {
    if(nodes.size() == 1) {
        map.put(nodes[0], bounds);
    } else {
        int totalWeight = 0;
        ...
        Vector<Node*> leftPartNodes, rightPartNodes;
        int leftPartWeight = 0;

        for(int i = 0; i < nodes.size(); i++) {
            if(leftPartWeight < totalWeight / 2 && i < nodes.size() - 1) {
                leftPartWeight += nodes[i]->weight;
                leftPartNodes.add(nodes[i]);
            } else {
                rightPartNodes.add(nodes[i]);
            }
        }

        double alpha = static_cast<double>(leftPartWeight) / totalWeight;
        Rectangles rectangles = splitRectangle(bounds, alpha);

        partitionChildrenOfHelper(leftPartNodes, rectangles.one, map);
        partitionChildrenOfHelper(rightPartNodes, rectangles.two, map);
    }
}

Map<Node*, Rectangle> partitionChildrenOf(Node* node, const Rectangle& bounds) {
    if(node == nullptr || node->children.size() == 0) {
        error("can't partition children of the node");
    }

    Map<Node*, Rectangle> result;
    partitionChildrenOfHelper(node->children, bounds, result);
    return result;
}

void formTreemapOfHelper(Node* root, const Rectangle& bounds, Map<Node*, Rectangle>& map) {
    map.put(root, bounds);

    if(root->children.size() != 0) {
        Map<Node*, Rectangle> childPartitionMap = partitionChildrenOf(root, bounds);
        map += childPartitionMap;

        for(Node* child : root->children) {
            if(child->children.size() != 0) {
                formTreemapOfHelper(child, map.get(child), map);
            }
        }
    }
}
...
Map<Node*, Rectangle> formTreemapOf(Node* root, const Rectangle& bounds) {
    if(root == nullptr) {
        error("can't form treemap");
    }

    Map<Node*, Rectangle> result;
    formTreemapOfHelper(root, bounds, result);
...
Node* loadGovernmentData(istream& in, const string& name) {
    ...
    Map<string, PriorityQueue<Node*>> agencyToAccounts;

    for(int i = 0; i < csv.numRows(); i++) {
        string accountName = csv[i]["federal_account_name"];
        string agencyName = csv[i]["owning_agency_name"];
        double outlay = stod(csv[i]["gross_outlay_amount"]);

        if(outlay > 0) {
            Node* account = new Node(accountName, outlay);
            agencyToAccounts[agencyName].enqueue(account, -outlay);
        }
    }

    Node* root = new Node(name, 0);
    PriorityQueue<Node*> orderedAgencies;

    for(string agencyName : agencyToAccounts) {
        double agencyWeight = 0;
        Node* agencyNode = new Node(agencyName, agencyWeight);
        PriorityQueue<Node*> accounts = agencyToAccounts[agencyName];

        while(!accounts.isEmpty()) {
            Node* accountNode = accounts.dequeue();
            agencyWeight += accountNode->weight;
            agencyNode->children.add(accountNode);
        }

        agencyNode->weight = agencyWeight;
        orderedAgencies.enqueue(agencyNode, -agencyWeight);
    }

    while(!orderedAgencies.isEmpty()) {
        Node* agencyNode = orderedAgencies.dequeue();
        root->weight += agencyNode->weight;
        root->children.add(agencyNode);
    }

    if(root->children.size() == 0) {
        delete root;
        error("no account spend any money!");
    }
