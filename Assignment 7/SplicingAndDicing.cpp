...
Nucleotide* findFirst(Nucleotide* dna, Nucleotide* target) {
    ...
    Nucleotide* dnaOuterPtr = dna;
    while(dnaOuterPtr != nullptr) {
        if(dnaOuterPtr->value == target->value) {
            Nucleotide* targetPtr = target->next;
            if(targetPtr == nullptr) {
                return dnaOuterPtr;
            }

            Nucleotide* result = dnaOuterPtr;
            Nucleotide* dnaInnerPtr = dnaOuterPtr->next;

            while(targetPtr != nullptr) {
                if(dnaInnerPtr == nullptr) {
                    return nullptr;
                }

                if(dnaInnerPtr->value == targetPtr->value) {
                    dnaInnerPtr = dnaInnerPtr->next;
...
bool spliceFirst(Nucleotide*& dna, Nucleotide* target) {
    ...
    Nucleotide* removedFirst = findFirst(dna, target);
    if(removedFirst == nullptr) {
        return false;
    }

    Nucleotide* spliceStart = removedFirst->prev;
    Nucleotide* ptr = removedFirst;
    ...
    Nucleotide* spliceEnd = ptr;

    if(spliceStart == nullptr) {
        if(spliceEnd == nullptr) {
            dna = nullptr;
        } else {
            dna = spliceEnd;
            dna->prev->next = nullptr;
            dna->prev = nullptr;
        }
    } else {
        removedFirst->prev = nullptr;

        if(spliceEnd == nullptr) {
            spliceStart->next = nullptr;
        } else {
            spliceEnd->prev->next = nullptr;
            spliceStart->next = spliceEnd;
            spliceEnd->prev = spliceStart;
        }
    }

    deleteNucleotides(removedFirst);
