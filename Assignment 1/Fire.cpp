...
void decreaseTemp(Grid<int>& fire, int i, int j) {
    if(i == fire.numRows() - 1) {
        return;
    }
    if(randomChance(double(2) / 3)) {
        if(fire.get(i, j) > 0) {
            fire[i][j] -= 1;
        }
    }
...
void copyTemp(Grid<int>& fire, int i, int j) {
    int k = i;
    int l = j;
    if(fire.numCols() == 1) {
        k = i - 1;
        fire[k][l] = fire[i][j];
    } else if (j == 0 || j == (fire.numCols() - 1)) {
        if(randomBool()) {
            k = i - 1;
            fire[k][l] = fire[i][j];
        } else {
