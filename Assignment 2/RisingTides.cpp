...
inline bool isWaterHigher(double height, const Grid<double>& terrain, int gridRowNum, int gridColNum) {
    return height >= terrain.get(gridRowNum, gridColNum);
}

void processCadinalDirection(double height, const Grid<double>& terrain, Grid<bool>& result, const GridLocation& grid
                             , Queue<GridLocation>& locationQueue, const Grid<bool>& locationAlreadyProcessed) {
    int terrainRows = terrain.numRows();
    int terrainCols = terrain.numCols();

    auto processNeighbour = [&](int row, int col) {
        if(row >= 0 && row < terrainRows && col >= 0 && col < terrainCols) {
            if(!locationAlreadyProcessed.get(row, col) && (height >= terrain.get(row, col))) {
                result.set(row, col, true);
                locationQueue.enqueue(GridLocation{row, col});
            }
        }
    };

    processNeighbour(grid.row, grid.col + 1);
    processNeighbour(grid.row + 1, grid.col);
    processNeighbour(grid.row, grid.col - 1);
    processNeighbour(grid.row - 1, grid.col);
}
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> locationAlreadyProcessed = {terrain.numRows(), terrain.numCols()};
    Queue<GridLocation> locationToBeProcessed;
    for(GridLocation source : sources) {
        if(isWaterHigher(height, terrain, source.row, source.col)) {
            result.set(source.row, source.col, true);
            locationToBeProcessed.enqueue(GridLocation{source.row, source.col});
        }

        while(!locationToBeProcessed.isEmpty()) {
            GridLocation grid = locationToBeProcessed.dequeue();
            locationAlreadyProcessed.set(grid.row, grid.col, true);
            processCadinalDirection(height, terrain, result, grid, locationToBeProcessed, locationAlreadyProcessed);
        }
    }
    return result;
}
