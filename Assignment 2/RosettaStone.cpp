...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) {
        error("kGramLength is not positive");
        return result;
    }
    ...
    for(int i = 0; i < str.length()  + 1 - kGramLength; i++) {
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    PriorityQueue<string> sortedStringList;
    for(string key : source) {
        sortedStringList.enqueue(key, source[key]);
        if(numToKeep-- <= 0) {
            sortedStringList.dequeue();
        }
    }

    while(!sortedStringList.isEmpty()) {
        string key = sortedStringList.dequeue();
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.isEmpty()) {
        error("corpus set is empty.");
    }

    double maxSimilarityScore(0);
    string result = "";
    for(Corpus corpus : corpora) {
        double similarityScore = cosineSimilarityOf(textProfile, corpus.profile);
